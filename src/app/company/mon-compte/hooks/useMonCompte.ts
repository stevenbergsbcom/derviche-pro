...
/** Hook principal de la page Mon Compte compagnie — chargement, sauvegarde profil et mot de passe.
 *  Calqué sur `src/app/admin/mon-compte/hooks/useMonCompte.ts` — seul le préfixe des logs diffère. */
export function useMonCompte() {
  ...
  const loadUserData = useCallback(async () => {
    try {
      ...
      if (profileResult.error) {
        logger.error('[CompanyMonCompte] Erreur chargement profil', profileResult.error);
        return;
      }
      ...
      logger.error('[CompanyMonCompte] Erreur inattendue', err as Error);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        logger.error('[CompanyMonCompte] Erreur sauvegarde', error);
        return;
      }
      ...
      logger.error('[CompanyMonCompte] Erreur', err as Error);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      logger.error('[CompanyMonCompte] Erreur changement mdp', err as Error);
