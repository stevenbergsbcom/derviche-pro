...
export interface ReservationModificationEmailData {
  /** Email du destinataire */
  to: string;
  /** Prénom et nom du destinataire */
  guestFullName: string;
  ...
  /** Nom de la compagnie */
  companyName: string;
  /** Ancienne date formatée */
  oldSlotDateFormatted: string;
  /** Ancienne heure formatée */
  oldSlotTimeFormatted: string;
  /** Nouvelle date formatée */
  newSlotDateFormatted: string;
  /** Nouvelle heure formatée */
...
/**
 * Générer le HTML de l'email de modification de créneau
 */
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Modification de votre réservation — Derviche Diffusion</title>
</head>
<body style="margin:0;padding:0;background-color:#f4f4f5;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f5;padding:32px 0;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <!-- En-tête bleu modifié -->
          <tr>
            <td style="background-color:#1e40af;padding:32px 40px;text-align:center;">
              <p style="margin:0;color:#bfdbfe;font-size:13px;font-weight:600;letter-spacing:2px;text-transform:uppercase;">Derviche Diffusion</p>
              <h1 style="margin:12px 0 0 0;color:#ffffff;font-size:24px;font-weight:700;">Créneau modifié ✓</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 40px 0 40px;">
              ...
              <p style="margin:12px 0 0 0;color:#374151;font-size:15px;line-height:1.6;">
                Votre réservation pour <strong>${safeShowTitle}</strong> a bien été mise à jour avec le nouveau créneau ci-dessous.
              </p>
            </td>
          </tr>
            ...
            <td style="padding:24px 40px 0 40px;">
              ...
              <div style="background-color:#f3f4f6;border-radius:8px;border:1px solid #e5e7eb;padding:14px 18px;">
                <p style="margin:0;font-size:14px;color:#9ca3af;text-decoration:line-through;">
                  ${data.oldSlotDateFormatted} à ${data.oldSlotTimeFormatted}
                </p>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:12px 40px 0 40px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0"
                style="background-color:#eff6ff;border-radius:8px;border:1px solid #bfdbfe;overflow:hidden;">
                <tr>
                ...
                <tr>
                  <td style="padding:16px 24px;border-bottom:1px solid #bfdbfe;">
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="50%" style="vertical-align:top;">
...
/**
 * Envoyer un email de modification de créneau de réservation
 * À appeler uniquement côté serveur (API routes)
 */
export async function sendReservationModificationEmail(
  ...
  try {
    ...
    logger.info('[email] Modification envoyée', {
      messageId: result?.id,
      reservationId: data.reservationId,
    });
