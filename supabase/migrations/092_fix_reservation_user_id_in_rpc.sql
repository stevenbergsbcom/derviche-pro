-- ============================================
-- Migration 092: Restaurer user_id dans create_public_reservation
-- Derviche Diffusion
-- Date: 2026-03-13
-- ============================================
-- PROBLÈME :
--   La migration 074 (verrou atomique FOR UPDATE) a réécrit le RPC
--   en oubliant d'inclure user_id dans l'INSERT.
--   Résultat : les réservations faites par un pro connecté ont user_id = NULL,
--   et sont traitées comme des réservations guest → prompt de rapatriement.
--
-- CORRECTION :
--   Ajouter user_id (= v_current_user_id = auth.uid()) dans l'INSERT.
--   - Pro connecté : user_id = UUID → visible dans le dashboard
--   - Anonyme : auth.uid() = NULL → réservation guest classique
-- ============================================
...
BEGIN
  ...
  --    la même transaction).
  --    FIX 092 : user_id = v_current_user_id (auth.uid()) pour lier
  --    la réservation au compte si l'utilisateur est connecté.
  -- ============================================
...
COMMENT ON FUNCTION public.create_public_reservation IS
  ...
  'user_id = auth.uid() si connecté, NULL sinon. '
  'Retourne CAPACITY_FULL:N si le créneau est complet. '
  'Bloque les admins. Inclut p_country.';
