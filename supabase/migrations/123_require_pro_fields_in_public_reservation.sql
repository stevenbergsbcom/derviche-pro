-- ============================================
-- Migration 123: Rendre obligatoires structure / ville / code postal
--                à la création d'une réservation publique
-- Derviche Diffusion
-- Date: 2026-06-05
-- ============================================
-- CONTEXTE :
-- Le client a reçu des réservations qui semblaient venir de festivaliers et
-- non de professionnels — la cause identifiée : les champs « Structure »,
-- « Ville » et « Code postal » étaient optionnels sur le formulaire public.
-- En les rendant obligatoires, on filtre la quasi-totalité des cas non
-- professionnels (un visiteur lambda ne renseignera pas spontanément la
-- structure pour laquelle il travaille).
--
-- PÉRIMÈTRE :
-- - On ajoute 3 RAISE EXCEPTION dans `create_public_reservation` (création).
-- - On NE touche PAS à `update_reservation_safe` (l'édition reste libre,
--   pour permettre le rattrapage des résa historiques avec champs vides).
-- - Adresse rue, pays, fonction et AFC restent optionnels (hors-scope client).
--
-- DÉFENSE EN PROFONDEUR :
-- L'UI publique + le dialog admin de création ajoutent déjà la validation
-- côté client. Cette migration garantit qu'un appel direct à la RPC
-- (script tiers, curl) ne contourne pas la règle.
-- ============================================
...
BEGIN
  ...
  -- Migration 123 : 3 nouveaux champs obligatoires pour filtrer les
  -- réservations non-professionnelles.
  IF p_organization IS NULL OR TRIM(p_organization) = '' THEN
    RAISE EXCEPTION 'La structure / organisation est obligatoire';
  END IF;

  IF p_postal_code IS NULL OR TRIM(p_postal_code) = '' THEN
    RAISE EXCEPTION 'Le code postal est obligatoire';
  END IF;
...
COMMENT ON FUNCTION public.create_public_reservation IS
  ...
  'Champs obligatoires (mig 123) : prénom, nom, email, structure, code postal, ville. '
